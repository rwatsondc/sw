...
for key in inData:
    ...
    if inData[key]['count']>filter:
        ...
        degWidth = np.interp(inData[key]['count'],[0,maxCount],[0,90])
...
curTime - minTime = radius
        ...
        radius = (curTime-minTime).days
        bars = ax.bar(curTheta,radius, width=thWidth, color=color,bottom=bottom, alpha=0.6)
        #break
#plt.axis('off')
plt.show()
